...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        for i in range(query_layer.size(0)):
            for j in range(query_layer.size(1)):
                data = query_layer[i][j][:][:].tolist()
                model.fit(data)
                # 分类中心点坐标
                ...
                for n in range(len(listResult)):
                    for q in range(len(listResult[n])):
                        query_layer[i][j][listResult[n][q]][:] = query_layer[i][j][listResult[n][0]][:]
